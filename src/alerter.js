...
if (card) {
await sendToChannel(‘conviction’,
card.text.replace(‘SWING TRADE’, ‘CONVICTION TRADE – CARD B (9DTE)’),
card.ticker
);
}
}
...
// HIGH CONVICTION FLOW – generate dual cards for cluster alerts
if (flowConviction.isHighConviction) {
// Card A – flow expiry (exact strike and expiry from flow data)
const flowExpiry = parsed.expiry;
const resA = await resolver.resolveContractWithExpiry(parsed.ticker, parsed.type, flowExpiry);
if (resA) {
const cardA = buildStratCard(resA.symbol, { mid: resA.mid, bid: resA.bid, ask: resA.ask, mode: ‘SWING’, dte: resA.dte }, resA);
if (cardA) {
await sendToChannel(‘flow’,
cardA.text.replace(‘SWING TRADE’, ‘CARD A – FLOW EXPIRY (’ + flowExpiry.slice(5).replace(’-’, ‘/’) + ‘)’),
cardA.ticker
);
}
}
...
```
// Card B -- Stratum 9DTE rules
const resB = await resolver.resolveContract(parsed.ticker, parsed.type, 'SWING');
if (resB) {
  const cardB = buildStratCard(resB.symbol, { mid: resB.mid, bid: resB.bid, ask: resB.ask, mode: 'SWING', dte: resB.dte }, resB);
  if (cardB) {
    await sendToChannel('flow',
      cardB.text.replace('SWING TRADE', 'CARD B -- STRATUM 9DTE'),
      cardB.ticker
    );
  }
}
