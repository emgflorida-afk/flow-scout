...
async function getScoreModifier(ticker, tradeDirection) {
  try {
    ...
    if (isBull && result.direction === 'sell') {
      return {
        modifier: -1,
        note: ' Congress SOLD ' + ticker + ' in last 30 days  against your direction',
      };
    }
    ...
    if (isBear && result.direction === 'buy') {
      return {
        modifier: -1,
        note: ' Congress BOUGHT ' + ticker + ' in last 30 days  against your direction',
      };
    }
